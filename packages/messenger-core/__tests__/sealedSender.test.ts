...
describe('Sealed Sender — AAD binding (Round 5 / S1)', () => {
  ...
  // OM-05 / NA-GATE-3 — a drain-time re-seal now carries the COMPOSE time in
  // aad.ts. This pins the window that carry is calibrated against: an
  // overnight-old ts must pass at default bounds, so a future narrowing of
  // SEALED_AAD_MAX_AGE_MS fails here rather than in the field.
  it('OM-05 — a 6h-old carried compose ts is accepted at default bounds', () => {
    const composed = 1_700_000_000_000;
    const wire = sealPayload('cert.cert.cert', 'x', {
      aad: {to: {userId: 'bob', deviceId: 1}, ts: composed},
    });
    const sealed = unsealPayload(wire);
    const r = verifySealedAad({
      sealed, selfUserId: 'bob', selfDeviceId: 1,
      now: composed + 6 * 3600_000,
    });
    expect(r.ok).toBe(true);
  });
